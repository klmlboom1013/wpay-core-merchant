...
public class PersistenceAdapterAspect {
    ...
    @After("execution(* com.wpay.core.merchant.adapter.out.persistence.*PersistenceAdapter.*Run(..))")
    public void after(JoinPoint joinPoint) {
        log.debug("[After] => {}", joinPoint.getSignature().getName());
    }

    @AfterReturning(pointcut = "execution(* com.wpay.core.merchant.adapter.out.persistence.*PersistenceAdapter.*Run(..))", returning = "result")
    public void afterReturning(JoinPoint joinPoint, Object result) {
        log.debug("[AfterReturning] => {} [result] => {}", joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.wpay.core.merchant.adapter.out.persistence.*PersistenceAdapter.*Run(..))", throwing = "e")
